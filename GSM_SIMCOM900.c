...
void GSM_Send_Character(char Data_Tx)
{
	while (!USART_GetFlagStatus(USART3, USART_FLAG_TXE));
	USART3->DR = Data_Tx;
}

void GSM_Send_String(char *Data_Tx)
{
	char Element;
	while (*Data_Tx != '\0')
	{
		Element = *Data_Tx;
		GSM_Send_Character(Element);
		Data_Tx++;
	}
}
...
int main()
{
	...
	GSM_Send_String("AT+CPIN?\r\n");
	Delay(3500000);

	GSM_Send_String("AT+CSQ\r\n");
	Delay(3500000);

	GSM_Send_String("AT+COPS?\r\n");
	Delay(3500000);

	GSM_Send_String("AT+CGMI\r\n");
	Delay(3500000);

	GSM_Send_String("AT+CGMM\r\n");
	Delay(3500000);

	GSM_Send_String("AT+CGSN\r\n");
	Delay(3500000);

	GSM_Send_String("ATD+919560016501;\r\n");
	Delay(3500000);
